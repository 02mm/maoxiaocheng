...
from nonebot import on_command, CommandSession,on_request,on_notice
...
@on_command('ai_chat',aliases='mxc2022')
async def test(session: CommandSession):
    await session.send('报告主人，我还活着，请勿担心！')
...
async def ai_chat(session: CommandSession):
    ...
    #跳过特殊功能，不使用智能对话
    function_chat = ['天气', '翻译:', '翻译：','名字','你叫什么','使用','帮助','请假：','请假:']
    for i in function_chat:
        if i in message:
            # print('\n\nok\n\n')
            # 结束本次对话
